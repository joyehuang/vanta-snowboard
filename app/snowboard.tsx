...
function boardTexture(color: string, back = false, length = 156) {
 ...
 c.fillStyle=back?'#141a1b':'#ecf0df';c.font='bold 24px monospace';c.textAlign='center';c.fillText('AM—01',256,back?1700:1670);c.font='16px monospace';c.fillText(`ALL MOUNTAIN / ${length}`,256,1820);
 const tex=new THREE.CanvasTexture(canvas);tex.colorSpace=THREE.SRGBColorSpace;return tex;
}
export default function Snowboard({color,reset,length=156,width=252,shape='Twin',finish='Satin',view='Studio'}:{color:string;reset:number;length?:number;width?:number;shape?:BoardShape;finish?:BoardFinish;view?:BoardView}){
 ...
 useEffect(()=>{
  ...
  const face=new THREE.MeshPhysicalMaterial({map:boardTexture(color,false,length),roughness:.36,metalness:.25,clearcoat:.7});
  const edge=new THREE.MeshPhysicalMaterial({color:'#b8bcb1',metalness:.9,roughness:.28});
  const baseMat=new THREE.MeshPhysicalMaterial({map:boardTexture(color,true,length),roughness:.4,side:THREE.BackSide});
  ...
  return()=>{cancelAnimationFrame(frame);observer.disconnect();visibility.disconnect();host.removeEventListener('pointerdown',down);host.removeEventListener('pointermove',move);host.removeEventListener('pointerup',up);host.removeEventListener('pointercancel',up);host.removeEventListener('keydown',keyboard);scene.traverse(o=>{if(o instanceof THREE.Mesh){o.geometry.dispose();const mats=Array.isArray(o.material)?o.material:[o.material];mats.forEach(m=>{m.map?.dispose();m.dispose();});}});renderer.dispose();host.removeChild(renderer.domElement);gsap.killTweensOf(board.rotation);gsap.killTweensOf(board.scale);materials.current.forEach(m=>gsap.killTweensOf(m));groupRef.current=null;materials.current=[];meshes.current=[];};
 // Board geometry is created once; color is updated independently below.
 // eslint-disable-next-line react-hooks/exhaustive-deps
 },[]);
 useEffect(()=>{materials.current.forEach((m,i)=>{m.map?.dispose();m.map=boardTexture(color,i===1,length);m.needsUpdate=true;});},[color,length]);
 useEffect(()=>{
 ...
 return <div ref={ref} className="snowboard-canvas" tabIndex={0} role="img" aria-label={`Interactive VANTA snowboard, ${length} cm, ${width} mm, ${shape}, ${finish}. Drag or use arrow keys to rotate.`}>{failed&&<div className="webgl-fallback">VANTA AM-01<br/><small>3D preview requires a WebGL-enabled browser.</small></div>}</div>
}
